...
00000000004010f4 <phase_6>:
  ...
  401100:	49 89 e5             	mov    %rsp,%r13				# %r13 = %rsi = %r14 = %rbp = a
  401103:	48 89 e6             	mov    %rsp,%rsi
  401106:	e8 51 03 00 00       	call   40145c <read_six_numbers>
  40110b:	49 89 e6             	mov    %rsp,%r14				
  40110e:	41 bc 00 00 00 00    	mov    $0x0,%r12d
	# pass if 1 <= a[0] <= 6 && a[i] != a[i + 1]
  401114:	4c 89 ed             	mov    %r13,%rbp							# ap %rbp = %r13 = (a + 1)..(
  401117:	41 8b 45 00          	mov    0x0(%r13),%eax			# *%r13
  40111b:	83 e8 01             	sub    $0x1,%eax				# a[0] - 1
  40111e:	83 f8 05             	cmp    $0x5,%eax
  401121:	76 05                	jbe    401128 <phase_6+0x34>	# jump if a[0] <= 6 & a[0] >= 1
  401123:	e8 12 03 00 00       	call   40143a <explode_bomb>	# NOTE: explode if a[0] < 1 | a[0] > 6
  401128:	41 83 c4 01          	add    $0x1,%r12d				# %r12d = 1..6
  40112c:	41 83 fc 06          	cmp    $0x6,%r12d				# jump if %12d == 6
  401130:	74 21                	je     401153 <phase_6+0x5f> # jump forward to next stage
	# check a[0] == a[1..5]
  401132:	44 89 e3             	mov    %r12d,%ebx				# %ebx = %r12d = 1..5
  401135:	48 63 c3             	movslq %ebx,%rax				# significant extend int to long
  401138:	8b 04 84             	mov    (%rsp,%rax,4),%eax		# a[%ebx] => %eax
  40113b:	39 45 00             	cmp    %eax,0x0(%rbp)			# compare a[0] : a[1..5]
  40113e:	75 05                	jne    401145 <phase_6+0x51>	# jump if a[0] != a[1]
  401140:	e8 f5 02 00 00       	call   40143a <explode_bomb>	# NOTE: explode if a[0] == a[1..5]
  401145:	83 c3 01             	add    $0x1,%ebx				# index %ebx from 2..5
  401148:	83 fb 05             	cmp    $0x5,%ebx
  40114b:	7e e8                	jle    401135 <phase_6+0x41>	# jump if %ebx > 5
	# 
  40114d:	49 83 c5 04          	add    $0x4,%r13				# ap %r13 = (a + 1)..(a + 
  401151:	eb c1                	jmp    401114 <phase_6+0x20>	# jump back
	# a[i] = 7 - a[i]
	# LOOP: init
  401153:	48 8d 74 24 18       	lea    0x18(%rsp),%rsi				# a.end
  401158:	4c 89 f0             	mov    %r14,%rax							# %rax = a.begin
  40115b:	b9 07 00 00 00       	mov    $0x7,%ecx							#	%edx = %ecx = 7
	# LOOP: body
  401160:	89 ca                	mov    %ecx,%edx
  401162:	2b 10                	sub    (%rax),%edx						# *%rax = 7 - *%rax (%rax = a..(a + 5)
  401164:	89 10                	mov    %edx,(%rax)
  401166:	48 83 c0 04          	add    $0x4,%rax							# %rax += 4
  40116a:	48 39 f0             	cmp    %rsi,%rax							# compare %rax to a.end
  40116d:	75 f1                	jne    401160 <phase_6+0x6c>	# jump to loop body if %rax != a.end

	# LOOP: init
  40116f:	be 00 00 00 00       	mov    $0x0,%esi							# %esi = 0 (scaled to point int or node *)
  401174:	eb 21                	jmp    401197 <phase_6+0xa3>	# jump forward to loop body 0
	# WHILE: body
  401176:	48 8b 52 08          	mov    0x8(%rdx),%rdx					# %rdx = %rdx->next (nodes[%eax - 1])
  40117a:	83 c0 01             	add    $0x1,%eax							# ++%eax
  40117d:	39 c8                	cmp    %ecx,%eax							# compare %eax : a[%rsi] (loop %eax - 1)
  40117f:	75 f5                	jne    401176 <phase_6+0x82>  
  401181:	eb 05                	jmp    401188 <phase_6+0x94>	# jump foward to loop body 1
	# IF: a[%rsi] <= 1
  401183:	ba d0 32 60 00       	mov    $0x6032d0,%edx					# %rdx = nodes
	# LOOP: body 1
  401188:	48 89 54 74 20       	mov    %rdx,0x20(%rsp,%rsi,2)	# b[%rsi] = nodes[a[%rsi] - 1]
  40118d:	48 83 c6 04          	add    $0x4,%rsi							# ++%rsi
  401191:	48 83 fe 18          	cmp    $0x18,%rsi							# compare %rsi : 6
  401195:	74 14                	je     4011ab <phase_6+0xb7>	# jump forward if %rsi == 6
	# LOOP: body 0
  401197:	8b 0c 34             	mov    (%rsp,%rsi,1),%ecx			# %ecx = a[%rsi] (%rsi = 0..5)
  40119a:	83 f9 01             	cmp    $0x1,%ecx							# compare %ecx : 1
  40119d:	7e e4                	jle    401183 <phase_6+0x8f>	# jump back if %ecx <= 1
	# IF: a[%rsi] > 1
	# WHILE: init
  40119f:	b8 01 00 00 00       	mov    $0x1,%eax							# %eax = 1
  4011a4:	ba d0 32 60 00       	mov    $0x6032d0,%edx					# %rdx = nodes
  4011a9:	eb cb                	jmp    401176 <phase_6+0x82>	# jump back to while body
	# LOOP: end (%rsi == 6) (all a[i] <= 1)

	# LOOP: init
  4011ab:	48 8b 5c 24 20       	mov    0x20(%rsp),%rbx				# %rbx = b[0]
  4011b0:	48 8d 44 24 28       	lea    0x28(%rsp),%rax				# %rax = nodes + 1
  4011b5:	48 8d 74 24 50       	lea    0x50(%rsp),%rsi				# %rsi = nodes.end
  4011ba:	48 89 d9             	mov    %rbx,%rcx							# %rcx = %rbx = nodes
	# LOOP: body
  4011bd:	48 8b 10             	mov    (%rax),%rdx						# %rdx = *%rax = nodes[1..5]
  4011c0:	48 89 51 08          	mov    %rdx,0x8(%rcx)					# %rcx->next = %rdx = nodes[1]..
  4011c4:	48 83 c0 08          	add    $0x8,%rax							# ++%rax
  4011c8:	48 39 f0             	cmp    %rsi,%rax
  4011cb:	74 05                	je     4011d2 <phase_6+0xde>	# jump to next stage if %rax = nodes.end
  4011cd:	48 89 d1             	mov    %rdx,%rcx							# %rcx = %rdx
  4011d0:	eb eb                	jmp    4011bd <phase_6+0xc9>	# jump back to loop body

	# LOOP: init
  4011d2:	48 c7 42 08 00 00 00 	movq   $0x0,0x8(%rdx)					# assign nullptr
  4011d9:	00 
  4011da:	bd 05 00 00 00       	mov    $0x5,%ebp							# %ebp = 5
	# LOOP: body
	# %rbx = nodes
  4011df:	48 8b 43 08          	mov    0x8(%rbx),%rax					# %rax = %rbx->next
  4011e3:	8b 00                	mov    (%rax),%eax						# %eax = nodes[1].val
  4011e5:	39 03                	cmp    %eax,(%rbx)						# compare %rbx->val : %rbx->next->val
  4011e7:	7d 05                	jge    4011ee <phase_6+0xfa>	# jump forward to update if node->val >= node->next->val
  4011e9:	e8 4c 02 00 00       	call   40143a <explode_bomb>	# explode if nodes[i] < nodes[i + 1]
	# LOOP: update
  4011ee:	48 8b 5b 08          	mov    0x8(%rbx),%rbx					# p = p->next
  4011f2:	83 ed 01             	sub    $0x1,%ebp							# --%ebp
  4011f5:	75 e8                	jne    4011df <phase_6+0xeb>	# jump to loop body if %ebp != 0
