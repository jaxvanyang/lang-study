...
class Matrix {
 public:
  ...
  friend bool ModifyMatrix(Matrix &m);  // 在 linear_main.cc 中声明和定义
  ...
  // 因为 class Matrix 在 class Vector 后声明，所以在 Vector 的方法不能使用
  // Matrix 所以把线性变换放在 Matrix 中 由于要修改参数 v，所以不用 const 修饰
